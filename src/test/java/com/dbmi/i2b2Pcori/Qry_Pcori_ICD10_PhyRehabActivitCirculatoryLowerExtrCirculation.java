...
public class Qry_Pcori_ICD10_PhyRehabActivitCirculatoryLowerExtrCirculation extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })
	public String qryICD10PhyRehabActivitCirculatoryLowerExtrCirculation() throws Exception {
		testLogin();
		Reporter.log(
...
//		ventilation, respiration and circulation		
		WebElement fromElement = driver.findElement(By.xpath(
				".//*[@title='ICD10PCS_2015AA \\ Physical rehabilitation and diagnostic audiology (procedure) "
						+ "\\ rehabilitation (procedure) \\ activities of daily living assessment \\ "
						+ "circulatory system - lower back / lower extremity \\ ventilation, respiration and circulation - ICD10PCS:F026G']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryICD10PhyRehabActivitCirculatoryLowerExtrCirculation().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB("select count(distinct patient_num) from observation_fact "
				...
				+ "\\(F02) Physical~ujz5\\(F026) Physica~vb8h\\(F026G) Physic~2s1r\\%') ");
	}
	...
	public void DBduration() throws Exception {
		testDB_duration(
				...
				+ "where concept_path LIKE '\\ICD10PCS?_2015AA\\(F) Physical r~vf8d\\(F0) Physical~kuq4\\"
				+ "(F02) Physical~ujz5\\(F026) Physica~vb8h\\(F026G) Physic~2s1r\\%' {ESCAPE '?'} ) "
