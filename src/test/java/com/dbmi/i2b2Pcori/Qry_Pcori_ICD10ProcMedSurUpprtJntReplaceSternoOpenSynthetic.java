...
public class Qry_Pcori_ICD10ProcMedSurUpprtJntReplaceSternoOpenSynthetic extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })
	public String qryICD10ProcMedSurUpprtJntReplaceSternoOpenSynthetic() throws Exception {
		testLogin();
		Reporter.log(
				"Query: ICD10 Procedures \\ Medical and surgical (procedure) "
				+ "\\ upper joints (procedure) \\ replacement \\ "
				+ "sternoclavicular joint, left \\ open");
...
//		sternoclavicular joint, left
		...
		//		Open
		driver.findElement(By
				.xpath(".//*[contains(@title, 'ICD10PCS_2015AA \\ Medical and surgical (procedure) \\ "
				+ "upper joints (procedure) \\ replacement \\ sternoclavicular joint, left "
				+ "\\ open - ICD10PCS:0RRF0')]"))
				.click();
		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
...
//		Synthetic substitute
		WebElement fromElement = driver.findElement(By.xpath(
				".//*[@title='ICD10PCS_2015AA \\ Medical and surgical (procedure)"
				+ " \\ upper joints (procedure) \\ replacement \\ sternoclavicular joint, left "
				+ "\\ open \\ synthetic substitute - ICD10PCS:0RRF0J']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryICD10ProcMedSurUpprtJntReplaceSternoOpenSynthetic().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB("select count(distinct patient_num) from observation_fact "
				...
				+ "where CONCEPT_PATH like '\\ICD10PCS?_2015AA\\(0) Medical an~5b7n\\"
				+ "(0R) Medical a~kfod\\(0RR) Medical~9tfx\\"
	...
	public void DBduration() throws Exception {
		testDB_duration(
				...
				+ "'\\ICD10PCS?_2015AA\\(0) Medical an~5b7n\\"
				+ "(0R) Medical a~kfod\\(0RR) Medical~9tfx\\"
				+ "(0RRF) Medical~28qk\\(0RRF0) Medica~0lf2\\"
				+ "(0RRF0J) Medic~4afj\\%' {ESCAPE '?'} ) "
