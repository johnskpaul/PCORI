...
public class Qry_Pcori_DemRaceWhiteDate extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })

	public String qryDemRaceWhiteDate() throws Exception {
		testLogin();
		Reporter.log("Query: Demographics/Race = White Date range: 1/1/2015 – 1/1/2017");
		// Demographics
...
//				.findElement(By.xpath(".//*[@title='Demographic \\ Race \\ White - DEM|RACE:W']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryDemRaceWhiteDate().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB("select count(distinct patient_num) from PATIENT_DIMENSION "
				+ "where race_cd = 'DEM|RACE:white'"
	...
	public void DBduration() throws Exception {
		testDB_duration("insert into dev02i2b2demodata.QUERY_GLOBAL_TEMP "
				+ "(patient_num, panel_count) with t as ( select e.patient_num "
				...
				+ "where race_cd = 'DEM|RACE:white')  "
				+ "AND  ( e.start_date >= to_date('01-Jan-2015 00:00:00','DD-MON-YYYY HH24:MI:SS') "
