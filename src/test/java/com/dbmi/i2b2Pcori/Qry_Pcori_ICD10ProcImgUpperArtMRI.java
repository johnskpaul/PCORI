...
public class Qry_Pcori_ICD10ProcImgUpperArtMRI extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })
	public String qryICD10ProcImgUpperArtMRI() throws Exception {
		testLogin();
		Reporter.log(
...
//		magnetic resonance imaging (mri)			
		WebElement fromElement = driver.findElement(By.xpath(
				".//*[@title='ICD10PCS_2015AA \\ Imaging (procedure) \\ "
						+ "upper arteries (procedure) \\ magnetic resonance imaging (mri) - ICD10PCS:B33']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryICD10ProcImgUpperArtMRI().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB("select count(distinct patient_num) from observation_fact "
				...
				+ "where CONCEPT_PATH like '\\ICD10PCS_2015AA\\(B) Imaging (p~8jdh\\(B3) "
				+ "Imaging @~rfcy\\(B33) Imaging~uh4z\\%')");
	}
	...
	public void DBduration() throws Exception {
		testDB_duration(
				...
				+ "Imaging @~rfcy\\(B33) Imaging~uh4z\\%' {ESCAPE '?'} ) "
