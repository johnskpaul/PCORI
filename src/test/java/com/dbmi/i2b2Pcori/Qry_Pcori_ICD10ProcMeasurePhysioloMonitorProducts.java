...
public class Qry_Pcori_ICD10ProcMeasurePhysioloMonitorProducts extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })
	public String qryICD10ProcMeasurePhysioloMonitorProducts() throws Exception {
...
//		ICD10 Procedures
		...
		driver.findElement(By
				.xpath(".//*[contains(@title, 'ICD10PCS_2015AA \\ "
						+ "Measurement and monitoring (procedure) - ICD10PCS:4')]"))
				.click();
		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
...
//		products of conception, cardiac 
		WebElement fromElement = driver.findElement(By.xpath(
				".//*[@title='ICD10PCS_2015AA \\ Measurement and monitoring (procedure) \\ "
						+ "physiological systems (procedure) \\ monitoring \\ products of conception, cardiac - ICD10PCS:4A1H']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryICD10ProcMeasurePhysioloMonitorProducts().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB(
				...
				+ "where CONCEPT_PATH like '\\ICD10PCS_2015AA\\(4) Measuremen~ge9w\\(4A) Measureme~dxgz\\(4A1) Measurem~49bf\\(4A1H) Measure~3za2\\%')" 
				);
	}
	...
	public void DBduration() throws Exception {
		testDB_duration(
				...
				+ "(4) Measuremen~ge9w\\(4A) Measureme~dxgz\\(4A1) "
				+ "Measurem~49bf\\(4A1H) Measure~3za2\\%' {ESCAPE '?'} ) "
