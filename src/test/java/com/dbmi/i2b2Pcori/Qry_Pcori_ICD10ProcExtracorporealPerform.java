...
public class Qry_Pcori_ICD10ProcExtracorporealPerform extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })
	public String qryICD10ProcExtracorporealPerform() throws Exception {
		testLogin();
		Reporter.log(
				"*******************************************Query: ICD10 Procedures \\ "
				+ "Extracorporeal assistance and performance (procedure) "
				+ "\\*******************************************");
...
// 		Extracorporeal assistance and performance (procedure)
				...
				".//*[@title='ICD10PCS_2015AA \\ Extracorporeal assistance "
				+ "and performance (procedure) - ICD10PCS:5']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryICD10ProcExtracorporealPerform().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB("select count(distinct patient_num) from observation_fact "
				...
				+ "where CONCEPT_PATH like '\\ICD10PCS_2015AA\\(5) Extracorpo~y4l1\\%') ");
	}
	...
	public void DBduration() throws Exception {
		testDB_duration(
				...
				+ "where concept_path LIKE '\\ICD10PCS?_2015AA\\(5) Extracorpo~y4l1\\%' {ESCAPE '?'} ) "
