...
public class Qry_Pcori_ICD10ProcNucMedCentralNerv extends LoginI2b2Pcori {

	@Test(groups = { "UI_query" })
	public String qryICD10ProcNucMedCentralNerv() throws Exception {
		testLogin();
		Reporter.log(
				"Query: ICD10PCS_2015AA/Nuclear medicine (procedure) / "
				+ "central nervous system (procedure)");
...
//		Central N..system		
		WebElement fromElement = driver.findElement(By.xpath(
				...
				+ "\\ Nuclear medicine (procedure) \\ central nervous system (procedure) - ICD10PCS:C0']/div"));
	...
	public void testCompare() throws Exception {
		if (!qryICD10ProcNucMedCentralNerv().contains(testDB())) {
	...
	public String testDB() throws ClassNotFoundException, SQLException {
		return Connect2DB("select count(distinct patient_num) from observation_fact "
				...
				+ "where CONCEPT_PATH like '\\ICD10PCS_2015AA\\"
				+ "(C) Nuclear me~bmw7\\(C0) Nuclear m~nc8j\\%') ");
	}
	...
	public void DBduration() throws Exception {
		testDB_duration(
				...
				+ "\\(C) Nuclear me~bmw7\\(C0) Nuclear m~nc8j\\%' {ESCAPE '?'} ) "
